# api 폴더 안에
# calculation.py, json_validate.py, models.py, preparation.py, preprocess.py, run.py, __init__.py
# 파일을 만들어 놓는다.
#
# PEP8 작성법으로 자동 형식화 한다.
# (venv) PS C:\flaskbook\ml_api\api> isort preparation.py preprocess.py calculation.py
# (venv) PS C:\flaskbook\ml_api\api> black preparation.py preprocess.py calculation.py

# analysis.ipynb 파일의 코드를 그대로 각 모듈에 복사, 붙여 넣기를 한 내용이다.
# 리펙터링 후 코드 임! (쥬피터에 내용과 조금 다름)

# ============================================
# 표준 라이브러리 import
# ============================================

import uuid  # 고유 ID(file_id) 생성용
from pathlib import Path  # 파일/디렉터리 경로 처리

from flask import abort, current_app, jsonify  # Flask 유틸
from sqlalchemy.exc import SQLAlchemyError  # DB 예외 처리

from .models import ImageInfo, db  # SQLAlchemy 모델 및 DB 세션
...
def load_filenames(dir_name: str) -> list[str]:
    """
    손글씨 문자 이미지가 저장된 디렉터리에서
    지정된 확장자의 파일명만 읽어 리스트로 반환
    """
